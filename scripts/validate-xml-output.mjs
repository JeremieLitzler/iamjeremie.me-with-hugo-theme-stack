...
function checkProlog(file, buf) {
  if (buf.length >= 3 && buf[0] === 0xef && buf[1] === 0xbb && buf[2] === 0xbf) {
    return fail(file, "starts with a UTF-8 BOM; the <?xml declaration must be the first bytes");
  }
  const text = buf.toString("utf8");
  if (!text.startsWith("<?xml")) {
    const idx = text.indexOf("<?xml");
    const lead = idx === -1 ? "(no <?xml declaration found)" : JSON.stringify(text.slice(0, idx));
    return fail(file, `must start exactly with "<?xml"; found ${lead} before the declaration`);
  }
  return true;
}
...
function checkWellFormed(file, text) {
  ...
  while (i < n) {
    ...
    if (text.startsWith("<?", i)) {
      ...
      if (end === -1) return fail(file, "unterminated processing instruction / declaration");
      i = end + 2;
      ...
      const top = stack.pop();
      if (top !== name) return fail(file, `tag mismatch: </${name}> closes <${top ?? "nothing"}>`);
      i = end + 1;
    } else {
      const end = text.indexOf(">", i);
...
function checkSitemap(file, text) {
  if (!/<urlset[\s>]/.test(text)) return fail(file, "missing <urlset> root element");
  const locs = [...text.matchAll(/<loc>([^<]*)<\/loc>/g)].map((m) => m[1]);
  if (locs.length === 0) return fail(file, "no <loc> entries found");
  const bad = locs.filter((u) => !u.startsWith(`${BASE_URL}/`));
  if (bad.length) {
    return fail(file, `${bad.length} <loc> URL(s) not under ${BASE_URL} (e.g. ${bad[0]})`);
  }
  return true;
}
